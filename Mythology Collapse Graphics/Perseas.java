...
public class Perseas  {
	...
	private void helmetTale() { //the method that represents the story to get the helmet
	       panel.setMessage("<html> Για να αποκτήσει την περικεφαλαία του Άδη η Αθήνα του κάνει την εξής ερώτηση: <br>"
			...
			if(choice==1) {
			  ...
			  if(choice==2) {
				  panel.setMessage("<html> Οι στρατιωτες του Αελα παραμένουν πίσω από τις πρόχειρές οχυρώσεις τους. Ο Αβδηρος αναγκάζεται να επιτεθεί πρώτος. <br>" 
						  ...
						  +"O Περσέας εντωπίζει τον Άβδηρο τραυματισμένο στο πεδίο της μάχης:  <br> <html>");
				  panel.addMessage("<html> 1) Αποφασίζει να τον σκοτώσει τον Άβδηρο <html>");
				  panel.addMessage("<html> 2) Παίρνει τον Άβδηρο όμηρο <html>");
				  choice = q.takeAnAnswer(2);
				  if(choice==1) {
		...
		if(sword) {
			...
			if(choice1 == hammercounter) {
				...
				if(choice1 == 2) {
					...
					panel.addMessage("<html> 1. Ο Θεός Άρης <html>");
	...
	public void eastStory() { //eastStory is a substory of the sackTale where the player goes to the East
		...
		while (choice1 != 3) {
			if (choice1 == 1 ) {
				    ...
				    panel.addMessage("<html> 4. Ο Άνιος <html>");
					choice1=q.takeAnAnswer(4);
					if (choice1 ==4 ) {
