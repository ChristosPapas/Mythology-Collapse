...
public class Odysseus {
	...
	private static void lotofagoi() {
		System.out.println("Oι κατάκοποι από τον πόλεμο ναύτες χαροπάλευαν μέρες ολόκληρες με τα κύματα,"
				...
				+ "\nμέρους.’ρπαξαν όλοι, λοιπόν, από μερικούς καρπούς και άρχισαν να τους καταβροχθίζουν. Έτσι κι ο Οδυσσέας, "
	...
	private static void help() {
		...
		takeAnAnswer(2);
		if(answer==1) {
			availabilityCheckRestore(); //infinite loop saver
			do {
				rand = randValue.nextInt(20)+1;
			} while (availableQuestions[rand-1]==false); //check for an available question
			availableQuestions[rand-1] = false;
			
			if(questionPicker (rand)==true) {  //getQuestion prints the question and returns true is the answer is true or false for the opposite situation 
	...
	//έκβαση ανάλογα την απάντηση
		...
		private static void teiresias() {
			System.out.println("Επόμενη στάση στο ατέλειωτο ταξίδι ειναι ο ’δης. Εκει συναντάει τον μάντη τειρεσία ο οποίος έχει ένα πολύ σημαντικό χρησμό για την εξέλιξη της ιστορίας.\n"
	...
	//έκβαση ανάλογα την απάντηση
		...
		private static void ilios () {
			...
			int answer2= answer;
				...
				takeAnAnswer(2);
				if (answer == 1) {
					System.out.println("Ο Δίας τον στέλνει με ευνοϊκούς ανέμους στην Ιθάκη αλλά λίγο πριν φτάσει εκεί, μαθαίνει από μια τυχαία συζήτηση αγνώστων, ότι ο αγαπημένος του γιος,\n"
							+ " ο Τηλέμαχος, είναι νεκρός από άγνωστη για τους κατοίκους του νησιού αιτία...\n\n");
					ithaki();
				} else {
